...
#import "FLAudioTool.h"
#import "FLMusicsTool.h"
#import <AVFoundation/AVFoundation.h>


@interface FLPlayingViewController ()<AVAudioPlayerDelegate>
 ...
 *  当前正在播放的音乐
 */
@property (nonatomic, strong) FLMusic *playingMusic;
/**
 *  当前播放器
 */
...
@implementation FLPlayingViewController
...
-(void)show{
    // 0.判断是否切换歌曲
    if (self.playingMusic != [FLMusicsTool playingMusic]) {
        // 重置数据
...
-(void)resetPlayingMusic{
    ...
    // 停止当前正在播放的歌曲
    [FLAudioTool stopMusicWithFilename:self.playingMusic.filename];
}
/// 开始播放
-(void)startPlayingMusic{
    // 执行动画完毕, 开始播放音乐
    // 1.取出当前正在播放的音乐模型
    FLMusic *music=[FLMusicsTool playingMusic];
    // 2.播放音乐
    self.player=[FLAudioTool playMusicWithFilename:music.filename];
    ...
    // 记录当前正在播放的音乐
    self.playingMusic=[FLMusicsTool playingMusic];
    // 3.设置其他属性
    // 设置歌手
...
-(void)addProgressTimer{
    ...
    // 2.创建定时器
    self.progressTimer=[NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(updateCurrentProgress) userInfo:nil repeats:YES];
    // 3.将定时器添加到时间循环
...
- (void)updateCurrentProgress{
    // 1.计算进度
    double process=self.player.currentTime / self.player.duration;
    
    // 2.获取滑块移动的最大距离
    double sliderMaxX=self.view.width-self.slider.width;
    
    // 3.设置滑块移动的位置
    self.slider.x=sliderMaxX*process;
...
- (IBAction)onProgressBgTap:(UIGestureRecognizer *)sender {
    ...
    // 3.计算进度
    double process=point.x/sender.view.width;
    // 4.设置播放时间
    self.player.currentTime=process * self.player.duration;
    // 5.立即更新
    [self updateCurrentProgress];
}
...
-(IBAction)previous{
    ...
    [self resetPlayingMusic];
    // 2.设置当前播放的音乐
    [FLMusicsTool setPlayingMusic:[FLMusicsTool previousMusic]];
    // 3.开始播放
    [self startPlayingMusic];
}
/**
...
-(IBAction)next{
    ...
    [self resetPlayingMusic];
    // 2.设置当前播放的音乐
    [FLMusicsTool setPlayingMusic:[FLMusicsTool nextMusic]];
    // 3.开始播放
    [self startPlayingMusic];
}
/**
...
- (IBAction)playOrPause{
    ...
    if (self.playOrPauseButton.selected) {
        ...
        // 调用工具类方法暂停
        [FLAudioTool pauseMusicWithFilename:self.playingMusic.filename];
    }else{
        // 未选中, 显示播放-->显示的暂停
        self.playOrPauseButton.selected=YES;
        [self startPlayingMusic];
